...
class QuantumDataPlotterApp:
    ...
    def load_file(self):
        ...
        try:
            ...
            # Ensure 'last_update_date' exists and convert to datetime with UTC
            if 'last_update_date' not in self.dataframe.columns:
                messagebox.showerror("Error", "CSV must contain 'last_update_date' column.")
                self.dataframe = None
                return

            # Convert to datetime, forcing UTC to handle mixed timezones
            self.dataframe['last_update_date'] = pd.to_datetime(self.dataframe['last_update_date'], utc=True)
            # Optional: Drop timezone info if you don't need it, keeping the time values consistent
            # self.dataframe['last_update_date'] = self.dataframe['last_update_date'].dt.tz_localize(None)
    ...
    def get_filtered_data(self):
        ...
        # Check if 'last_update_date' is actually a datetime type now
        ...
        # Create a column for date-hour (using UTC time)
        # dt.floor('H') works on timezone-aware datetime objects
        df['date_hour'] = df['last_update_date'].dt.floor('H')

        # Group by date-hour and get the first row of each group
        # sort_values ensures we get the first occurrence chronologically if there are ties within the hour
        filtered_df = df.sort_values('last_update_date').groupby('date_hour').first().reset_index()
